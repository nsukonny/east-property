  ...
  var initMinMaxFields = () => {
    ...
    minMaxWrappers.forEach((wrapper) => {
      ...
      apply == null ? void 0 : apply.addEventListener("click", () => {
        ...
        }).format(Number(Math.floor(minValue / 1e3)));
        const drawMaxValue = new Intl.NumberFormat("en-AE", {
          maximumFractionDigits: 0
        }).format(Number(Math.floor(maxValue / 1e3)));
        minMaxText.textContent = `${minValue >= 1e3 ? drawMinValue + "k" : minValue} - ${maxValue >= 1e3 ? drawMaxValue + "k" : maxValue}`;
