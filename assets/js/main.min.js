(() => {
  ...
  var PropertyMap = class {
    ...
    initPointSelection(AdvancedMarkerElement, PlaceAutocompleteElement) {
      ...
      (_c2 = this.latitudeInput) == null ? void 0 : _c2.addEventListener("change", () => {
        this.updateSelectionMarkerFromInputs(
          AdvancedMarkerElement
        );
      });
      (_d2 = this.longitudeInput) == null ? void 0 : _d2.addEventListener("change", () => {
        this.updateSelectionMarkerFromInputs(
          AdvancedMarkerElement
        );
      });
    ...
    updateCoordinateInputs(position) {
      if (this.latitudeInput) {
        this.latitudeInput.value = Number(position.lat).toFixed(8);
        this.latitudeInput.dispatchEvent(
          new Event("input", {
            bubbles: true
          })
        );
      ...
      if (this.longitudeInput) {
        this.longitudeInput.value = Number(position.lng).toFixed(8);
        this.longitudeInput.dispatchEvent(
          new Event("input", {
            bubbles: true
          })
        );
  ...
  var { __, _n, _x, sprintf } = window.wp.i18n;
  document.addEventListener("DOMContentLoaded", () => {
    "use strict";
  ...
  var initUploader = () => {
    ...
    pond.on("addfile", (err, file) => {
      if (err || userFiles.some((f) => Number(f.id) === Number(file.id))) return;
      userFiles.push({
        id: file.id,
    ...
    pond.on("removefile", (err, file) => {
      const idx = userFiles.findIndex((f) => Number(f.id) === Number(file.id));
      if (idx > -1) {
        uGrid.classList.add("preloader");
    ...
    document.addEventListener("click", (e) => {
      ...
      syncPresetsFromDom();
      const preset = presets.find((f) => Number(f.id) === Number(item.dataset.id));
      if (!preset) return;
    ...
    [uGrid, pGrid].forEach((grid) => {
      sortable_esm_default.create(grid, {
        ...
        onEnd: () => {
          ...
          if (grid === uGrid) {
            userFiles = newOrderIds.map((id) => userFiles.find((f) => Number(f.id) === Number(id))).filter(Boolean);
          } else {
            presets = newOrderIds.map((id) => presets.find((f) => Number(f.id) === Number(id))).filter(Boolean);
          }
          syncToHidden();
        }
      });
  ...
  var initIsCompleted = () => {
    ...
    if (!form) return;
    const isCompletedInput = form.querySelector('input[name="is_completed"]');
    if (!isCompletedInput) return;
    const deliveryDateInput = form.querySelector('input[name="delivery_date"]');
    if (!deliveryDateInput) return;
    isCompletedInput.addEventListener("change", () => {
      deliveryDateInput.disabled = !!isCompletedInput.checked;
    });
  };
