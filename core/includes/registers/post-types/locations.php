...
add_action( 'init', static function () {
	...
	/*
	 * The location taxonomy is not translated, so Polylang does not build the
	 * language prefixed variants of the rule above. Without them /ru/areas/{slug}/
	 * fell through to WordPress guessing a permalink: a 404, or a redirect to an
	 * unrelated developer that happens to share the slug.
	 */
	if ( function_exists( 'pll_languages_list' ) && function_exists( 'pll_default_language' ) ) {
		...
		foreach ( (array) pll_languages_list() as $language ) {
			...
			add_rewrite_rule(
				'^' . $language . '/areas/(?!page-[0-9]+/?$)([^/]+)/?$',
				'index.php?location=$matches[1]&lang=' . $language,
				'top'
			);
		}
	}
