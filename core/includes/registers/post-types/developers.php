...
function register_post_type_developers() {
	$labels = array(
		...
		'insert_into_item'      => __( 'Add into developer' , 'east-property' ),
...
/**
 * Add opening developer on developers page with pagination
 *
 * Old URL Example
 * http://eastproperty.local/developers/omniyat-properties/
 */
add_action( 'init', static function () {
	add_rewrite_rule(
		'^developers/([^/]+)/page-([0-9]+)/?$',
		'index.php?post_type=developers&name=$matches[1]&cur_page=$matches[2]',
		'top'
	);
} );