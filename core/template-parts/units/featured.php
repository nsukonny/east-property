...
$href          = $args['href'] ?? '';
$link_text     = $args['link_text'] ?? __( 'See all properties' , 'east-property' );
$card_template = $args['card_template'] ?? 'unit-card';
$border_top    = ! empty( $args['border_top'] );
$before        = $args['before'] ?? ( '<section class="properties' . ( $border_top ? ' has-border-top' : '' ) . '"><div class="container">' );
$after         = $args['after'] ?? '</div></section>';
$button_text   = $args['button_text'] ?? '';
$button_url    = $args['button_url'] ?? '';

$wrapper_classes = 'properties-wrapper' . ( $border_top ? ' border-top' : '' );
